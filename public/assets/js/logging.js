...
const LOG_PREFIX = "[WEB PERFORMANCE TRAINING]";
  ...
  if (
    ...
    console.table([
      {
        ...
        "% Total":
          Math.round(
            (metric.attribution.timeToFirstByte / metric.value) * 100
          ) + "%",
      },
      {
        ...
        "% Total":
          Math.round(
            (metric.attribution.resourceLoadDelay / metric.value) * 100
          ) + "%",
      },
      {
        ...
        "% Total":
          Math.round(
            (metric.attribution.resourceLoadDuration / metric.value) * 100
          ) + "%",
      },
      {
        ...
        "% Total":
          Math.round(
            (metric.attribution.elementRenderDelay / metric.value) * 100
          ) + "%",
      },
    ]);
  } else if (
    ...
    console.table([
      ...
      },
      {
        ...
        "% Total":
          Math.round((metric.attribution.firstByteToFCP / metric.value) * 100) +
          "%",
      },
    ]);
    ...
    console.log(
      `Interaction event type: %c${metric.attribution.interactionType}`,
      ...
      console.table([
        {
          ...
          "% Total":
            Math.round((metric.attribution.inputDelay / metric.value) * 100) +
            "%",
        },
        {
          ...
          "% Total":
            Math.round(
              (metric.attribution.processingDuration / metric.value) * 100
            ) + "%",
        },
        {
          ...
          "% Total":
            Math.round(
              (metric.attribution.presentationDelay / metric.value) * 100
            ) + "%",
        },
      ]);
    }
    ...
    if (metric.attribution.longAnimationFrameEntries) {
      const allScripts = metric.attribution.longAnimationFrameEntries
        .map((a) => a.scripts)
        .flat();
      ...
      if (allScripts.length > 0) {
        ...
        // line numbers.
        const scriptsBySource = sortedScripts.reduce(
          (acc, { sourceURL, duration }) => {
            if (sourceURL) {
              // Exclude empty URLs
        ...
        for (const [key, value] of Object.entries(scriptsBySource)) {
          console.log(
            `Script source link: ${key} (Duration${
              value.length > 1 ? "s" : ""
            }: ${value})`
          );
        }
    ...
    console.table([
      {
        ...
        "% Total":
          Math.round(
            (metric.attribution.waitingDuration / metric.value) * 100
          ) + "%",
      },
      {
        ...
        "% Total":
          Math.round((metric.attribution.cacheDuration / metric.value) * 100) +
          "%",
      },
      {
        ...
        "% Total":
          Math.round((metric.attribution.dnsDuration / metric.value) * 100) +
          "%",
      },
      {
        ...
        "% Total":
          Math.round(
            (metric.attribution.connectionDuration / metric.value) * 100
          ) + "%",
      },
      {
        ...
        "% Total":
          Math.round(
            (metric.attribution.requestDuration / metric.value) * 100
          ) + "%",
      },
    ]);
  }
